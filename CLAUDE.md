...
**Facebook Reels** (`poster/facebook.py`): **NOT operational — scaffolded only.** The code (3-phase `video_reels` upload, `cutter auth facebook`) exists, but posting to a Page needs `pages_manage_posts` at Advanced Access, which requires Meta App Review + business verification — not completed, and public posting is blocked without it. Facebook is therefore effectively off: it's skipped in the daily run unless credentials are present (they aren't). Do not describe cutter as posting to Facebook.
...
**Telegram** (`telegram.py`): Bot API with `getUpdates` long-polling — no webhook or server. Sends clips as video files directly (50 MB cap). Updates are journaled to `telegram_state.json` in the workdir because Telegram only retains unread updates ~24 h. Telegram is the sole approval/notification channel (no SMS or other third-party messaging service).
