...
**TikTok upload** (`poster/tiktok.py`): init → chunked PUT (10 MB) → poll status. Auto-refresh on 401. Two modes via `TIKTOK_POST_MODE`: `inbox` (default — uploads to the user's TikTok inbox as a draft to publish in-app, works with sandbox/unaudited apps; caption is sent via WhatsApp to paste) and `direct` (publishes immediately with `PUBLIC_TO_EVERYONE`, requires a TikTok-audited production app). TikTok rejects production audits for personal/internal tools, so sandbox + inbox mode is the supported path — see `docs/tiktok_oauth.md`.
